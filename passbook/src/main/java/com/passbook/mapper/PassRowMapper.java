...
public class PassRowMapper implements RowMapper<Pass> {
    ...
    private static byte[] ASSINGED_DATE = Constants.PassTable.ASSIGNED_DATE.getBytes();
    ...
    public Pass mapRow(Result result, int i) throws Exception {
       ...
       pass.setToken(Bytes.toString(result.getValue(FAMILY_I, TOKEN)));
        ...
        String conDataStr = Bytes.toString(result.getValue(FAMILY_I, CON_DATE));
        if (conDataStr.equals("-1")){
            pass.setConDate(null);
        }
        else {
            pass.setConDate(DateUtils.parseDate(conDataStr, patterns));
        }
